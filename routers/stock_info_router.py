...
from util.helper import get_paginated_list
...
@router.post("/info", summary="Get general stock information", tags=["Stock"])
async def get_stock_info(req: GlobalRequest, yf_service: StockInfoService = Depends(get_stock_info_service)):
    ...
    return success(stock_info)

@router.post("/price", summary="Get historical price data for a specified period and interval", tags=["Stock"])
...
@router.post("/price-max-history", summary="Get maximum historical price data with pagination", tags=["Stock"])
